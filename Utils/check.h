...
extern "C" {
	...
	uint8_t upxor(uint8_t* buf, int len); //UPIXELSЭ��У��
	uint8_t crc8(uint8_t* buf, int len); //MSPЭ��У��
	uint16_t hexin_crc16_compute(uint8_t *pSrc, int message_id, uint32_t len, uint16_t init); //MAVLINKУ��
